...
export default function SchoolsScreen({ data, t, dm, i18n, navigate }) {
  ...
  const [bookingStep, setBookingStep] = useState(0); // 0=browse, 1=school-detail, 3=checkout
  ...
  const [bookingStatus, setBookingStatus] = useState(null); // null | loading | success | cancelled | error
  const [bookingError, setBookingError] = useState(null);
  const [showTip, setShowTip] = useState(() => {
    try { 
      return !JSON.parse(localStorage.getItem("soulsurf_tooltips") || "{}")["schools-intro"]; 
    } catch { 
      return true; 
    }
  });
  ...
  const dismissTip = () => {
    setShowTip(false);
    try { 
      const tt = JSON.parse(localStorage.getItem("soulsurf_tooltips") || "{}"); 
      tt["schools-intro"] = true; 
      localStorage.setItem("soulsurf_tooltips", JSON.stringify(tt)); 
    } catch {}
  };

  const handlePayment = async () => {
    // Validation
    if (!selectedSchool || !selectedCourse || !bookingForm.name || !bookingForm.email || !bookingForm.date) {
      setBookingError("Bitte alle Pflichtfelder ausfüllen");
      return;
    }
    ...
    try {
      console.log("🔵 [Schools] Initiating payment:", {
      ...
      await redirectToCheckout({
        ...
        currency: (selectedSchool.priceRange.currency || "eur").toLowerCase(), // ← FIX: lowercase
        people: bookingForm.people,
        ...
        locale: i18n?.lang || "de",
        returnUrl: window.location.href, // ← FIX: Added returnUrl
      });
    } catch (err) {
      console.error("❌ [Schools] Payment error:", err);
      setBookingStatus("error");
      setBookingError(err.message || "Zahlung fehlgeschlagen");
    }
  };

  const sendBookingRequest = () => {
    // Non-binding request (no payment)
    console.log("📧 [Schools] Sending non-binding request:", bookingForm);
    
    // TODO: Send email to school via API endpoint
    // fetch('/api/send-booking-request', { ... })
    ...
    return (
      <div style={{ paddingTop: 24, animation: "screenIn 0.25s ease both" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: t.text2, margin: 0 }}>
              {selectedSchool.name} · {selectedCourse.name}
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button 
                onClick={() => setBookingForm({ ...bookingForm, people: Math.max(1, bookingForm.people - 1) })} 
                style={{ 
                  width: 36, 
                  height: 36, 
                  borderRadius: 10, 
              ...
              <button 
                onClick={() => setBookingForm({ ...bookingForm, people: Math.min(8, bookingForm.people + 1) })} 
                style={{ 
                  width: 36, 
                  height: 36, 
                  borderRadius: 10, 
          ...
          <div>
            ...
            <textarea 
              ...
              rows={3} 
              placeholder="Optional: Besondere Wünsche, Fragen..." 
              style={{ 
                width: "100%", 
                padding: "12px 14px", 
                borderRadius: 12, 
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <span style={{ fontSize: 13, color: t.text2 }}>
              {selectedCourse.name} × {bookingForm.people}
            </span>
            <span style={{ 
              fontFamily: "'Playfair Display', serif", 
              fontSize: 22, 
              ...
              {formatPrice(totalPrice, selectedSchool.priceRange.currency)}
            </span>
          </div>
        ...
        <button 
          ...
          disabled={!isReady || bookingStatus === "loading"} 
          style={{
            width: "100%", 
            padding: "16px", 
            borderRadius: 16, 
            ...
            fontFamily: "'Playfair Display', serif",
            background: isReady && bookingStatus !== "loading" 
        ...
        >
          {bookingStatus === "loading" 
            ? _("pay.processing") 
            : `${_("pay.payNow")} · ${formatPrice(totalPrice, selectedSchool.priceRange.currency)}`} {/* ← FIX: Syntax korrigiert */}
        </button>
        ...
        <button 
          ...
          style={{
            ...
            color: isReady ? t.accent : t.text3,
      ...
      <div style={{ paddingTop: 24, animation: "screenIn 0.25s ease both" }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ 
              ...
              {formatPrice(school.priceRange.from, school.priceRange.currency)}–
              {formatPrice(school.priceRange.to, school.priceRange.currency)}
            </span>
            {school.verified && (
              <span style={{ 
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
          {school.highlights.map(h => (
            <span key={h} style={{ 
              background: dm ? "rgba(0,150,136,0.08)" : "#E0F2F1", 
              color: t.accent, 
              padding: "6px 12px", 
              borderRadius: 20, 
              fontSize: 11, 
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: t.text2, marginTop: 2 }}>
                  {course.duration} · {course.groupSize}
                </div>
  ...
  return (
    <div style={{ paddingTop: 24, animation: "screenIn 0.25s ease both" }}>
      ...
      {spot && (
        <div style={{ 
          background: dm ? "rgba(30,45,61,0.6)" : "#F5F5F5", 
          borderRadius: 14, 
          padding: "12px 16px", 
          marginBottom: 16, 
          display: "flex", 
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: t.text2 }}>
              {spot.waveType} · {spot.season} · {spot.water} · 
              {spot.wetsuit === "none" ? _("g.noWetsuit") : spot.wetsuit}
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
              <span style={{ fontSize: 11, color: "#FFA000" }}>
                <StarRating rating={school.rating} size={11} /> {school.rating}
              </span>
              <span style={{ fontSize: 11, color: t.text3 }}>
                ({school.reviewCount})
              </span>
              <span style={{ fontSize: 11, color: t.accent, fontWeight: 600 }}>
                ab {formatPrice(school.priceRange.from, school.priceRange.currency)}
              </span>
            </div>
            <div style={{ display: "flex", gap: 4, marginTop: 4 }}>
              {school.languages.slice(0, 3).map(l => (
                <span key={l} style={{ 
                  fontSize: 9, 
                  background: t.inputBg, 
                  padding: "2px 6px", 
