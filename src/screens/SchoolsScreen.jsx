// SoulSurf – SchoolsScreen v6.6.1 (V1: Bugfix – Suitability Badge on Cards)
// Contextual routing: accepts navParams from Decision Engine
...
export default function SchoolsScreen({ data, t, dm, i18n, navigate, navParams, spotObj }) {
  const _ = i18n?.t || ((k, f) => f || k);
  // v6.6: Auto-select spot from navParams (Decision Engine context)
  ...
  // v6.6: Update selected spot when navParams change
  useEffect(() => {
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      ...
      {/* v6.6: Contextual banner from Decision Engine */}
      {fromDecision && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "#E65100" }}>{_("schools.decisionBanner", "Ein Coach hilft dir heute")}</span>
          </div>
          <div style={{ fontSize: 11, color: t.text2, lineHeight: 1.5 }}>
            {decisionReason ? _(decisionReason) : _("schools.decisionDesc", "Die Bedingungen sind heute anspruchsvoll – eine Surfschule sorgt für Sicherheit und Spaß.")}
          </div>
        </div>
      )}
      ...
      {/* Spot info with suitability badge */}
      {spot && (
        <div style={{ background: dm ? "rgba(30,45,61,0.6)" : "#F5F5F5", borderRadius: 14, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: t.text }}>{spot.name}</span>
              {suitability && <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: `${suitability.color}12`, color: suitability.color, fontWeight: 700, fontFamily: "'Space Mono', monospace" }}>{suitability.emoji} {_(suitability.labelKey)}</span>}
            </div>
            <div style={{ fontSize: 11, color: t.text2 }}>{spot.waveType} · {spot.season} · {spot.water}</div>
          </div>
        </div>
      )}
        ...
        <button key={school.id} onClick={() => { setSelectedSchool(school); setBookingStep(1); }} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
              ...
              {/* v6.6.1: Suitability badge per school card */}
