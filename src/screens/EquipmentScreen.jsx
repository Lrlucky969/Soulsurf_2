// SoulSurf – EquipmentScreen v7.6 (Design Upgrade Complete)
...
export default function EquipmentScreen({ data, t, dm, i18n }) {
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <h2 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 28, fontWeight: 800, color: t.text, marginBottom: 6 }}>{_("equip.title")}</h2>
      <p style={{ fontSize: 14, color: t.text2, marginBottom: 16 }}>Finde das perfekte Board für dein Level und Gewicht.</p>

      {exp === "zero" && (
        <div style={{ background: dm ? "rgba(14,165,233,0.1)" : "#E0F2FE", border: `1px solid ${dm ? "rgba(14,165,233,0.2)" : "#B2DFDB"}`, borderRadius: 14, padding: "12px 16px", marginBottom: 16 }}>
      ...
      <div style={{ background: dm ? "rgba(30,45,61,0.8)" : t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: "18px 20px", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <input type="range" min={40} max={120} value={weight} onChange={e => setWeight(Number(e.target.value))} style={{ flex: 1, accentColor: "#0EA5E9" }} />
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 700, color: t.accent, minWidth: 60 }}>{weight} kg</span>
        </div>
      ...
      {/* Volume Result */}
      <div style={{ background: dm ? "rgba(14,165,233,0.1)" : "#E0F2FE", borderRadius: 16, padding: "20px", textAlign: "center", marginBottom: 20, border: `1px solid ${dm ? "rgba(14,165,233,0.2)" : "#B2DFDB"}` }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: t.accent, textTransform: "uppercase", marginBottom: 4 }}>Empfohlenes Volume</div>
        <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 42, fontWeight: 800, color: t.accent }}>{rec.volume}L</div>
        <div style={{ fontSize: 12, color: t.text2 }}>bei {weight}kg · {EXPERIENCE_LEVELS.find(e => e.id === exp)?.label}</div>
      </div>

      {/* Board Recommendations */}
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: t.text3, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>Board-Empfehlungen</div>
        {rec.boards.map((b, i) => (
          <div key={i} style={{ display: "flex", gap: 12, padding: "14px 16px", background: b.best ? (dm ? "rgba(14,165,233,0.12)" : "#E0F2FE") : t.card, border: `1px solid ${b.best ? t.accent : t.cardBorder}`, borderRadius: 14, marginBottom: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: t.accent, marginTop: 2 }}>{b.volume}</div>
      ...
      <div style={{ background: dm ? "rgba(30,45,61,0.8)" : "#F5F5F5", borderRadius: 16, padding: "16px 18px", marginBottom: 20 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: t.text3, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>{_("equip.finSetup")}</div>
      ...
      {data.coaching.boardHint && (
        <div style={{ background: dm ? "rgba(255,183,77,0.1)" : "#FFF8E1", borderRadius: 14, padding: "14px 16px", border: "1px dashed #FFB74D" }}>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#E65100", textTransform: "uppercase", marginBottom: 4 }}>{_("equip.fromDiary")}</div>
