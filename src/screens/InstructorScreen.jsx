...
const STATUS_LABELS = {
  upcoming: { label: "Anstehend", color: "#42A5F5", emoji: "📅" },
  active: { label: "Aktiv", color: "#FFA726", emoji: "🏄" },
  completed: { label: "Abgeschlossen", color: "#66BB6A", emoji: "✅" },
};

export default function InstructorScreen({ data, auth, t, dm, navigate }) {
    ...
    return (
      <div style={{ paddingTop: 24 }}>
        ...
        <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: "16px", marginBottom: 12 }}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: t.text3, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>📝 Session-Notizen</div>
          <textarea
            value={editNotes || s.notes}
            onChange={e => setEditNotes(e.target.value)}
            onBlur={() => { if (editNotes) { updateSession(s.id, { notes: editNotes }); setEditNotes(""); } }}
            rows={4}
            placeholder="Notizen zur Session... (in deiner Sprache – wird automatisch übersetzt)"
          ...
          {s.notesTranslated && (
            <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 10, background: dm ? "rgba(66,165,245,0.08)" : "#E3F2FD", border: `1px solid ${dm ? "rgba(66,165,245,0.15)" : "#BBDEFB"}` }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: "#1565C0", marginBottom: 4 }}>🌐 Auto-Übersetzung (DE)</div>
        ...
        {/* Video Link */}
        <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: "16px", marginBottom: 12 }}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: t.text3, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>🎬 Video</div>
          <input
            type="url"
        ...
        {/* Rating */}
        <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: "16px", marginBottom: 12 }}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: t.text3, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>⭐ Bewertung des Schülers</div>
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 800, color: t.text, marginBottom: 4 }}>👨‍🏫 Instructor Mode</h2>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[
          ...
          { key: "upcoming", label: "📅 Anstehend", count: sessions.filter(s => s.status === "upcoming").length },
          { key: "active", label: "🏄 Aktiv", count: sessions.filter(s => s.status === "active").length },
          { key: "completed", label: "✅ Fertig", count: sessions.filter(s => s.status === "completed").length },
        ].map(f => (
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[
          { emoji: "👥", value: sessions.length, label: "Gesamt" },
          { emoji: "📅", value: sessions.filter(s => s.status === "upcoming").length, label: "Offen" },
          { emoji: "🎓", value: sessions.filter(s => s.status === "completed").length, label: "Zertifikate" },
        ].map((s, i) => (
          ...
          <button key={s.id} onClick={() => setSelectedSession(s.id)} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                ...
                {s.videoUrl && <span style={{ fontSize: 10, color: t.text3 }}>🎬 Video</span>}
