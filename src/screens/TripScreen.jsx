...
export default function TripScreen({ data, t, dm, i18n, spotObj, navigate }) {
  ...
  const countdown = useMemo(() => {
    ...
    if (diff < 0) return { text: _("trip.running"), emoji: "🏄", past: false };
    if (diff === 0) return { text: _("trip.todayStart"), emoji: "🎉", past: false };
      ...
      {currentTrip && (
        <>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {[{ id: "overview", label: _("trip.details") }, { id: "weather", label: _("trip.weather") }, { id: "packing", label: _("trip.packing") }].map(s => (
            ...
            <div style={{ animation: "screenIn 0.25s ease both" }}>
              ...
              <div style={{ background: dm ? "rgba(30,45,61,0.8)" : "linear-gradient(135deg, #FFF3E0, #FFF8E1)", border: `1px solid ${dm ? "rgba(255,183,77,0.15)" : "#FFE0B2"}`, borderRadius: 16, padding: "14px 18px", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                  ...
                  <</div>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 11, color: t.text2, display: "block", marginBottom: 3 }}>{_("trip.returnFlight")}</label>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 11, color: t.text2, display: "block", marginBottom: 3 }}>{_("trip.budget")}</label>
                  ...
                  <</div>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 11, color: t.text2, display: "block", marginBottom: 3 }}>{_("trip.notesLabel")}</label>
