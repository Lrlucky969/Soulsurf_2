  ...
  return (
    <div style={{ paddingTop: 24 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, overflowX: "auto" }}>
        {[
          { label: "Einträge", value: stats.count, emoji: "📝" },
          { label: "Ø Mood", value: stats.avgMood ? `${stats.avgMood}` : "–", emoji: stats.avgMood >= 4 ? "😊" : stats.avgMood >= 3 ? "😐" : "😕" },
          { label: "Top Welle", value: stats.topWave || "–", emoji: "🌊" },
          { label: "Fotos", value: stats.totalPhotos, emoji: "📷" },
        ].map((s, i) => (
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {[
          ...
          { id: "filled", label: "Ausgefüllt", count: Object.values(data.diary).filter(e => e && (e.whatWorked || e.whatFailed || e.notes || e.mood)).length },
          ...
          { id: "photos", label: "📷 Mit Fotos", count: Object.keys(diaryPhotos).filter(k => diaryPhotos[k]?.length > 0).length },
        ].map(f => (
                ...
                <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: "16px 18px", marginTop: 6, animation: "slideUp 0.25s ease both" }}>
                  ...
                  <div style={{ borderTop: `1px dashed ${dm ? "#2d3f50" : "#E0E0E0"}`, paddingTop: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      ...
                      <label style={{ fontSize: 11, fontWeight: 600, color: t.accent, background: dm ? "rgba(77,182,172,0.1)" : "#E0F2F1", border: `1px solid ${t.accent}`, borderRadius: 8, padding: "5px 12px", cursor: "pointer" }}>+ Foto hinzufügen<input type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={e => { if (e.target.files?.[0]) addPhoto(dayData.day, e.target.files[0]); e.target.value = ""; }} /></label>
                    </div>
                    ...
                    ) : <div style={{ fontSize: 12, color: t.text3, textAlign: "center", padding: "12px 0", background: t.inputBg, borderRadius: 10 }}>📷 Noch keine Fotos – halte deine Sessions fest!</div>}
                  </div>
                  {entry?.date && <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: t.text3, textAlign: "right", marginTop: 10 }}>📅 {new Date(entry.date).toLocaleDateString("de-DE", { day: "numeric", month: "long" })}</div>}
                </div>
              )}
            </div>
          </div>
